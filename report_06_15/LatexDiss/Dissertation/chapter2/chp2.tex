...
\section{Accelerate the algorithm with OPUs random features}
\label{section:OPU}
...
\section{The final form of $GSA-\varphi_{OPU}$ with OPUs' random features}
\todome{I will the next pseudo code, it is not final}
\begin{algorithm}[H]
\DontPrintSemicolon
  \KwInput{2-Classes labelled graph dataset $\mathcal{X}=(\G_i,y_i)_{i=1,\ldots,n}$}
  \KwOutput{Trained model to classify graphs}
  ...
  $\mathcal{D}_{\varphi_{OPU}}\gets (\varphi_i,Y_i)_{i=1,\ldots, n}$\\
