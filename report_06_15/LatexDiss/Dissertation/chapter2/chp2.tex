...
Let us recall a few notions about random features (RFs, see Section~\ref{sec:RF}). Let us assume that we have a psd (often shift-invariant \nt{shift invariant? how do you define this on $\R^{k \times k}$?}) kernel on $\R^{k \times k}$, that can be written in the form:
\begin{equation}
...
Although $\K$ technically takes the adjacency matrices of two graphlets as an input, we do not require it to be permutation-invariant, since it will be combined with the graph sampling process. \nt{I do not see how this is an argument. I would remove this paragraph.} For instance, we will see in experiments that even a simple Gaussian kernel on adjacency matrices performs well!
