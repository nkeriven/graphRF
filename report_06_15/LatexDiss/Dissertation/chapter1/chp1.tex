...
Two graphs $\mathcal{G}=(\mathcal{V},\mathcal{E})$ and $\mathcal{G'}=(\mathcal{V'},\mathcal{E'})$ are said to be \emph{isomorphic}, written $\mathcal{G}'\cong \mathcal{G}$, if there exists a bijective function $\mathcal{M}:\mathcal{V}\xrightarrow{} \mathcal{V}'$ such that $(u_i,u_j)\in \mathcal{E}$ iff $(\mathcal{M}(u_i),\mathcal{M}(u_j))\in \mathcal{E}'$. Deciding is two graphs are isomorphic is known to be a difficult problem: it is actually an open question if this problem is solvable in polynomial time or is NP-complete. This is equivalent to test if one adjacency matrix is a permutation of the other. In the worst case where the graphs to be compared do not have a specific structure which can used as \emph{a prior} in this test, then a brute force test would consider all the $k!$ permutation matrices. For small graphs, smarter test exist \citep{graphlet_kernel}, but this is an open question if this is true for general graphs. In the rest, we denote by  to the required time cost to test the isomorphism between two graphs of size $k$ with.

As we will see, the \emph{graphlet kernel} is defined by enumerating subgraphs of a given graph, also called \emph{graphlets}, and building a histogram of their frequency of apparition. We introduce some convenient notations below. Let us denote by $\mathcal{H}=\{\mathcal{H}_1,..., \mathcal{H}_{N_k}\}$ the set of all graphs of size $k$. Depending on the context, there is two choices in defining this set. Either we count all possible adjacency matrices and treat isomorphic graphs as different graphs, in which case we have $N_k=2^{k(k-1)/2}$ different graphlets. We refer to this case as graphlets \emph{with repetition}. Or we do not distinguish isomorphic graphs, so we have here $N_k<2^{k(k-1)/2}$ but it is still exponential in $k$. We call this case graphlets \emph{without repetition}. The classical graphlet kernel uses graphlets without repetition, which can require expensive isomorphism tests. We will see that some methods on graphlets \emph{with} repetition also perform well in practice.

To match a subgraph $F$ of size $k$ to the corresponding graphlet of the same size, we define the matching function $\varphi_{k}^{match}$. In the case of graphlets \emph{with} repetition, it is defined as: 
\[
\varphi_k^{match}(F) = \left[ 1_{(F = \phlet_i)}\right]_{i=1}^{N_k}= \left[ 1_{(\mathbf{A}_F = \mathbf{A}_{\phlet_i})}\right]_{i=1}^{N_k} \in \{0,1\}^{N_k}
\]
In other words, $\varphi_k^{match}$ puts a $1$ in the coordinate $i$ if the adjacency matrices of both graphs $F, \phlet_i$ are equal, and $0$ otherwise. Clearly the cost of each test is $k^2$ which is a polynomial function in $k$, and the global cost is $O(N_k k^2)$. On the other hand, when the case of graphlets \emph{without} repetition is considered, $\varphi_k^{match}$ is rather defined as:
\[
\varphi_k^{match}(F) = \left[ 1_{(F \cong \phlet_i)}\right]_{i=1}^{N_k} \in \{0,1\}^{N_k}
\]
which means that $\varphi_k^{match}$ puts a $1$ in the coordinate $i$ if $F\cong \phlet_i$, and $0$ otherwise. With our notations, a single evalutation costs $O(N_k C_k)$, although the $N_k$ here is smaller than the case with repetition.

%Let $\G=(\V,\E)$ be a graph of size $v$, and 
Let $\mathbf{F}=\{F_1,\ldots,F_s\}$ be a set of $s$ size-$k$ graphs. We define the counting function $\varphi_k^{hist}$ which counts, for each graphlet $\phlet_i$ in $\phlet$, how many matches it has in $\mathbf{F}$. Here, we can use either version of $\varphi_k^{match}$, and write:
\[
\varphi_k^{hist}(\mathbf{F})=\frac{1}{s}\sum_{j=1}^s \varphi_k^{match} (F_j) \in \mathbb{R}^{N_k}
\]
where the term $\frac{1}{s}$ is introduced for normalization purposes, such that $\varphi_k^{hist}(\mathbf{F})$ is a proper histogram. %With this definition and when $s$ is sufficiently large, $\varphi_k^{hist}(\mathbf{F}_\G)$ can be seen as the vector whose $i_{th}$ entry has the frequency of graphlet $\phlet_i$ in the graph $\G$.
...
For a graph $\mathcal{G}=(\V,\E)$, we define the vector $f_\mathcal{G}\in \mathcal{R}^{N_k}$ whose $i$-th entry equals the normalized-number of occurrences of $\mathcal{H}_i$ in $\mathcal{G}$. The vector $f_\G$ is usually referred to by the $k$-spectrum of $\G$. With our notations, defining $\mathbf{F}_\G=\{F_1,\ldots,F_s\}$ the set of all size-k subgraphs in $\G$ with $s=\tbinom{v}{k}$, we have:
\[
f_\G=\varphi_k^{hist}(\mathbf{F}_\G)
\]
where here graphlets are generally considered \emph{without} repetition in the literature. The graphlet kernel is just an inner product between the histograms.
...
\textbf{The computational cost} of approximating $k$-graphlet spectrum using graph sampling %the matching function $\varphi_k^{match}$, will be referred to with $\mathbf{GSA}-{\varphi_k^{match}}$, 
includes the cost of sampling a subgraph $C_s$, iterating over all $s$ sampled subgraphs $F_j\sqsubseteq \G$, iterating over all $N_k$ graphlets $\phlet_i$ and testing the isomorphism between each subgraph and each graphlet $F_i\cong \phlet_i$. So the computational cost of this method is:
\begin{equation}
\label{eq:cost_graphlet}
    C_{gk + gs}= O(C_s s N_k C_k)
\end{equation}
\todoNK{Very quickly, how does it compare to exhaustive enumeration ?}