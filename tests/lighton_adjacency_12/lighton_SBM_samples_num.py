...
nodes_num, features_num= range (3,7),750        
...
for (n_ind, nodes) in enumerate(nodes_num):
    ...
    for (num_ind, sam_num) in enumerate(samples_num):
        for i in range(num_expe):
            ...
            np.savetxt('Lighton_adjacency_SBM_uniform_samp_num1.csv', accur, delimiter=',',header=Header+"\n"+header)
            feat_map.close()
...
np.savetxt('SBM_Lighton_Adjacency_SBM_samples_num1.csv', np.array(result).flatten(), delimiter=',',header=header)
