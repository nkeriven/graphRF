...
nodes_num, features_num= range (3,7),5000        
samples_num=list(range (400,1000,100))+list(range(1000, 2501,300))
mult_factor=1.75
sampler_type="simple_random_sampling"                       
Header="instantanious saving of the results"
header="Adjacency Lighton random features with changing nodes_num and samples_num variables:\n features_num, mult_factor= 5000, 1.75\nnum_expe=1\n the sampler used is : uniform sampling"
num_expe=1                                                                
result=np.zeros((len(nodes_num), len(samples_num),num_expe))
dataset=dataset_loading()
accur=np.array([])
for (n_ind, nodes) in enumerate(nodes_num):
    sampler=graph_sampler(sampler_type=sampler_type, nodes_num=nodes)                     ### one change here: preprocess
    for (num_ind, sam_num) in enumerate(samples_num):
        for i in range(num_expe):
            print('{}/{},{}/{}, {}/{}'.format(n_ind+1, len(nodes_num),num_ind+1,len(samples_num),i+1, num_expe))
            (G_train,y_train),(G_test,y_test) = dataset.generate_SBM(mult_factor=mult_factor) # generate a new synthetic dataset for each expe
            feat_map=Lighton_random_features(nodes**2, features_num)       ### critical change: input shape was nodes_num**2
            GRF = graphlet_avg_features(sam_num, sampler, feat_map, batch_size=None, verbose=True)
            K_train,K_test=calc_kernel(G_train, G_test, GRF)
            result[n_ind,num_ind, i] = run_grid(K_train, K_test, y_train, y_test)
            accur=np.append(accur,result[n_ind, num_ind, i])
            np.savetxt('Lighton_adjacency_SBM_uniform_samp_num.csv', accur, delimiter=',',header=Header+"\n"+header)
            feat_map.close()
...
np.savetxt('SBM_Lighton_Adjacency_SBM_samples_num.csv', np.array(result).flatten(), delimiter=',',header=header)
