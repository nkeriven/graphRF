...
nodes_num, features_num,samples_num, p_flyback, sigma= range (3,7),5000, 2000, 0.85, 10 #################3########              
...
for (n_ind, nodes) in enumerate(nodes_num):
    ...
    for (f_ind, factor) in enumerate(mult_factor):
        for i in range(num_expe):
            ...
            np.savetxt('Lighton_adjacency_Uniform_sampling_.csv', accur, delimiter=',',header=Header+"\n"+header)


# In[ ]:


np.savetxt('SBM_Lighton_Adjacency_uniformsampling_.csv', np.array(result).flatten(), delimiter=',',header=header)
