...
np.random.seed(0)
nodes_num, features_num,samples_num, p_flyback, sigma= range (5,8),500, 2000, 0.85, 10 #################3########              
mult_factor=np.append(np.arange(1,1.1,0.02),np.arange(1.1,1.8,0.2))
...
for (n_ind, nodes) in enumerate(nodes_num):
    ...
    for (f_ind, factor) in enumerate(mult_factor):
        for i in range(num_expe):
            print('{}/{},{}/{}, {}/{}'.format(n_ind+1, len(nodes_num),f_ind,len(mult_factor),i+1, num_expe))
