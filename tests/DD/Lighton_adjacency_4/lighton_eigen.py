...
for (f_ind, feat) in enumerate(feat_axis):
    ...
    result[f_ind] = run_grid(K_train, K_test, y_train, y_test)
    feat_map.close()
...
np.savetxt('DD_Lighton_Adjacency.csv', np.array(result), delimiter=',',header=header)
