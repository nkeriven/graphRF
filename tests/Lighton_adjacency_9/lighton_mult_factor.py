...
nodes_num, features_num,samples_num, p_flyback, sigma= range (3,6),5000, 2000, 0.5, 10 #################3########              
mult_factor=1+np.linspace(0.2,2,9)
sampler_type,preprocess="random_walk_induced_graph_sampling","Eigen_values"                       
Header="instantanious saving of the results"
header="Adjacency Lighton random features with changing nodes_num and mult_factor variables:\n features_num, samples_num= 5000, 2000\nnum_expe=1\n the sampler used is : random walk induced graph"

######################################################
#feat_axis=[10, 25, 50, 100, 250, 500, 1000,5000]           ######################################3
num_expe=1                                                                
...
for (n_ind, nodes) in enumerate(nodes_num):
    ...
    for (f_ind, factor) in enumerate(mult_factor):
        for i in range(num_expe):
            ...
            accur=np.append(accur,result[n_ind,f_ind, i])
            np.savetxt('Lighton_adjacency_random_walk_SBM_RF.csv', accur, delimiter=',',header=Header+"\n"+header)
            feat_map.close()
...
np.savetxt('SBM_Lighton_Adjacency.csv', np.array(result).flatten(), delimiter=',',header=header)
