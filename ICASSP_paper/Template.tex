...
%
Given a graph $\G$ of size $v$, let $\mathfrak{F}_\G=\{\mathcal{F}_1,\mathcal{F}_2,\ldots,\mathcal{F}_{\binom{v}{x}}\}$ be the collection of subgraphs induced by all size-$k$ subsets of nodes. The following representation vector is called the $k$-spectrum of $\G$:
\begin{equation}
\label{eq:gk}
...
In this paper, we focus on the main remaining bottleneck of the graphlet kernel, that is, the function $\match$. We therefore define a framework where it is replaced with a user-defined map $\varphi: \bar{\mathfrak{H}} \to \mathbb{R}^m$, which leads to the final representation:
\begin{align}
	\label{eq:fhat_phi}
...
The resulting methodology is referred to as \emph{Graphlet Sampling and Averaging} (GSA-$\varphi$). The cost of computing \eqref{eq:fhat_phi} is essentially that of computing $\varphi$ for each sample: $C_{GSA-\varphi}= \mathcal{O}\left(s C_S C_{\varphi}\right)$, where $C_{\varphi}$ is the cost of applying $\varphi$.
%
...
Assume that we have a base kernel $\kappa(\F,\F')$ between \emph{graphlets}, with a RF decomposition $(\xi_\mathbf{w}, p)$, and define $\varphi$ as in \eqref{eq:def_RF}. Then, it is easy to show \cite{Keriven2017a, Keriven2018} that the Euclidean distance between the embeddings \eqref{eq:fhat_phi} approximates the following \emph{Maximum Mean Discrepancy} (MMD) \cite{Gretton2007, Sriperumbudur2010} between distributions on $\bar{\mathfrak{H}}$:
...
%
The proof of the theorem below can be found in the supplementary material. %It states  that the Euclidean distance between the representation vectors $\bld{z}_\G, \bld{z}_{\G'}$ converges to the MMD metric between their distributions $\bld{f}_{\G,S_k}$, $\bld{f}_{\G',S_k}$. 
...
\end{multline}
\end{theorem}
Hence, if two classes of graphs are well-separated in terms of the MMD \eqref{eq:MMD}, for sufficiently high $m,s$, GSA-$\varphi$ has the same classification power. However, according to \eqref{eq:bound}, the number of random features $m$ should be of the order of the number of samples $s$. We have seen that the latter is generally quite high, hence most usual random feature scheme, typically in $C_\varphi =O(k^2 m)$, still have a high computation cost. We discuss next the use of \emph{optical hardware}.

\subsection{Considered choices of $\varphi_{RF}$}
\label{sec:phi_choices}
\textbf{Gaussian maps}: the RF map of the Gaussian kernel \cite{rahimi2008random}. For a graphlet $\F$ we define its vectorized adjacency matrix  $\mathbf{a}_\F=flatten(\mathbf{A}_\F)$, then:
\begin{equation}
...
\end{equation}
where the $\mathbf{w}_j\in \R^{k^2}$ are drawn from a Gaussian distribution and $b_j \sim \mathcal{U}([0, 2\pi])$. While using a Gaussian kernel on $\mathbf{a}_\mathcal{F}$ is not very intuitive, this will serve as a baseline for the other methods. Note that $\varphi_{Gs}$ is not permutation-invariant.
\BlankLine
